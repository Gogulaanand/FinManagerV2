...
5. (Medium) `saveScenario` in `packages/sync/src/scenarios.ts` (lines 160-170) runs UPDATE-then-INSERT without a wrapping transaction, so a concurrent same-id insert can race between the statements.
...
6. (Low) `apps/web/src/components/theme-toggle.tsx:99` has the repo's one fully empty catch; make it `console.warn`.
7. (High) The D-038 first-load-zeros pattern is fixed in Goals but latent in the Portfolio and Expenses workspaces: queries mounted before the first PowerSync sync attach to an empty DB and render zeros until remount.
...
11. (Low) D-042 dead component code: the portfolio hubs no longer render the global event form, valuation form, ledger history card, or valuation history card, but the files were deliberately retained.
