...
- GitHub: no PR exists for the first batch; PR #18 remains open and separate. Production deployment is still `18bb2c8`.
...
Verification at this working tree: workspace build, tests, lint, typecheck and full Prettier check passed; core 220, sync 89, schema 43 and tokens 27 tests (379 total). Server/access tests: 17. Python backup-verifier tests: 2. Fresh isolated PostgreSQL reset and seven pgTAP files: 130 assertions. Deno 2.9.6 checks the Edge Function with its import map. Password recovery Playwright: 1 passed. Full authenticated CI/Preview E2E remains pending until the branch is pushed and reviewed.
...
1. Sign into the separate Codex browser with the staged test account, then verify the synthetic row and fresh initial sync there. Complete controlled client outage/recovery acceptance without stopping the shared provider.
...
4. Inactivity: owner selects existence notice, verified summary, or a full encrypted recovery package and designates test recipient(s). Current implementation supports the first two scopes; full encrypted package design is not implemented. Before deployment, finish provider delivery/bounce verification, external heartbeat monitoring and deliberate failure/receipt drills. Provider acceptance alone does not prove inbox delivery. Never simulate by actually sending to existing trusted contacts.
