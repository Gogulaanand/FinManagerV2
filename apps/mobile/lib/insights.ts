...
async function friendlyError(response: Response): Promise<Error> {
  try {
    ...
    if (value.message) return Object.assign(new Error(value.message), { code: value.error });
  } catch {
...
export function useInsights() {
  ...
  const data = useMemo(() => {
    ...
    const comparison = scenarios[0] ? computeTax(toTaxInput(scenarios[0].input)) : null;
  ...
  const buildDigest = useCallback(
    (scope: InsightScope): FinancialDigest =>
      buildFinancialDigest(scope, { ...data, month: currentMonth() }),
    [data],
  );

  const stream = useCallback(
    async (payload: Record<string, unknown>, onDelta: (text: string) => void) => {
      ...
      if (!response.ok) throw await friendlyError(response as unknown as Response);
