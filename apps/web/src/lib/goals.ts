...
export function useGoals(): GoalsApi {
  ...
  // Resolve a settings object for display: the stored row, or defaults with the
  // expense baseline auto-suggested from Phase 4 spend until the user saves one.
  ...
  const saveGoal = useCallback(
    async (input: Goal) => {
      ...
      return repoSaveGoal(db, userId, GoalSchema.parse({ ...input, userId }));
    },
    [db, userId],
  );
  ...
  const saveFireSettings = useCallback(
    async (input: FireSettings) => {
      if (!userId) return null;
      return repoSaveFireSettings(db, userId, FireSettingsSchema.parse({ ...input, userId }));
    },
    [db, userId],
  );
