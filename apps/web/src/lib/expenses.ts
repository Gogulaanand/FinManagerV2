...
export function useExpenses(): ExpensesApi {
  ...
  const accounts = useMemo(() => mapAccountRows(rowRecords(accountsResult.data ?? [])), [accountsResult.data]);
  const categories = useMemo(() => mapCategoryRows(rowRecords(categoriesResult.data ?? [])), [categoriesResult.data]);
  const transactions = useMemo(
    () => mapTransactionRows(rowRecords(transactionsResult.data ?? [])),
    [transactionsResult.data],
  );
  const budgets = useMemo(() => mapBudgetRows(rowRecords(budgetsResult.data ?? [])), [budgetsResult.data]);
  ...
  const summary = useMemo(() => calculateMonthlySummary(transactions, categories, month), [categories, month, transactions]);
  ...
  const saveTransaction = useCallback(
    async (transaction: Transaction) => {
      if (userId) await repoSaveTransaction(db, userId, transaction);
    },
    [db, userId],
  );
